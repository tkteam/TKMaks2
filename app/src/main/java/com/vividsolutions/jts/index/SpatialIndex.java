...
{
  /**
   * Adds a spatial item with an extent specified by the given {@link com.vividsolutions.jts.geom.Envelope} to the index
   */
  void insert(Envelope itemEnv, Object item);

  /**
   * Queries the index for all items whose extents intersect the given search {@link com.vividsolutions.jts.geom.Envelope}
  ...
  /**
   * Queries the index for all items whose extents intersect the given search {@link com.vividsolutions.jts.geom.Envelope},
   * and applies an {@link com.vividsolutions.jts.index.ItemVisitor} to them.
   * Note that some kinds of indexes may also return objects which do not in fact
   * intersect the query envelope.
   *
   * @param searchEnv the envelope to query for
