...
public class JsonToDatabase {
	...
	public void saveControlData(Gson gson, String json) {

		try {

			Type listType = new TypeToken<List<MissionControl>>() {
			}.getType();
			List<MissionControl> missionControlList = gson.fromJson(json, listType);

			for (MissionControl missionControl : missionControlList) {
				missionControl.Insert();
			}
	...
	public Boolean saveMissions(Activity act) {
		...
		try {
			...
			HttpRequestForJson httprequestforjson = new HttpRequestForJson(Info.tagSyncMissions, Tools.GetSyncDateRange(act));

			String json = httprequestforjson.getJson();
